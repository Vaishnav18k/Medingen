...
def create_tables_and_seed():
 ...
 # Create salts
            salts = [
                Salt(name="Paracetamol", description="Pain reliever and fever reducer."),
                Salt(name="Acetaminophen", description="Alternative name for Paracetamol."),
                Salt(name="Ibuprofen", description="Nonsteroidal anti-inflammatory drug."),
                Salt(name="Ibuprofen Lysine", description="Building block for making proteins in the body.")
           ]
...
# Create review and description
            review = Review(rating=5, comment="Best medicine for fever!")
            desc = Description(title="About", content="Paracetamol treats mild to moderate pain and fever.")
...
# Assign relationships
            paracetamol.salts.extend(salts[:2])  # First two salts
            ibuprofen.salts.extend(salts[2:])    # Last two salts
            paracetamol.reviews.append(review)
            paracetamol.descriptions.append(desc)

         # Assign salts to product
        # paracetamol.salts.extend ([salt1, salt2])
        ...
        # ibuprofen.salts = [salt3, salt4]
           ...
           # Add all objects
            db.session.add_all([paracetamol, ibuprofen, *salts, review, desc])
        # db.session.add_all([paracetamol,ibuprofen, salt1, salt2, salt3, salt4, review1, desc1])
            db.session.commit()
